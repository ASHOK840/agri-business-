...
const SalePaymentSection = ({ saleId }: SalePaymentSectionProps) => {
  ...
  const emptyForm: BuyerPaymentFormData = {
    saleId,
    amount: '',
  ...
  const [form, setForm] = useState<BuyerPaymentFormData>(emptyForm);

  const loadSummary = () => {
    setIsLoading(true);
    setLoadError('');
    getSalePaymentSummary(saleId)
      .then(setSummary)
      .catch((err) => setLoadError(err?.response?.data?.message || 'Could not load payment summary.'))
      .finally(() => setIsLoading(false));
  };

  useEffect(loadSummary, [saleId]);

  const handleChange =
    (field: keyof BuyerPaymentFormData) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const canRecordPayment = summary.finalSaleAmount !== null && summary.paymentStatus !== 'PAID';
  ...
  return (
    <div>
      ...
      {summary.finalSaleAmount === null ? (
        <p className="text-sm text-gray-400">
          Payments can be recorded once the delivery settlement is finalized.
        </p>
      ) : (
        <>
          <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100 mb-4">
            <div className="px-4 py-3 flex justify-between text-sm">
              <span className="text-gray-500">Final Sale Amount</span>
              <span className="text-gray-800 font-medium">
                {formatCurrency(summary.finalSaleAmount)}
              </span>
            </div>
            <div className="px-4 py-3 flex justify-between text-sm">
            ...
            <div className="px-4 py-3 flex justify-between text-sm bg-gray-50">
              ...
              <span
                className={`font-semibold ${(summary.outstandingAmount ?? 0) > 0 ? 'text-red-600' : 'text-green-700'}`}
              >
          ...
          {canRecordPayment && !isAdding && (
            <div className="bg-white rounded-lg border border-gray-200 p-4 flex items-center justify-between mb-4">
              <p className="text-sm text-gray-500">Record a payment received from the buyer.</p>
          ...
          {canRecordPayment && isAdding && (
            ...
            >
              <TextField
                ...
                step="0.01"
                min="0"
                max={summary.outstandingAmount ?? undefined}
                value={form.amount}
                onChange={handleChange('amount')}
                hint={`Outstanding: ${formatCurrency(summary.outstandingAmount ?? 0)}`}
                required
              />
              <TextField
                label="Payment Date *"
          ...
          {summary.payments.length === 0 ? (
            <p className="text-sm text-gray-400">No payments recorded yet for this sale.</p>
          ) : (
